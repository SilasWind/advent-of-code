...
function DayThirteen() {
  ...
  const doTheThing = () => {
    ...
    let newOutput = 0;
    const findSplit = (mirrorArray: string[]) => {
      ...
      for (let sideIndex = 0; sideIndex < mirrorArray[0].length; sideIndex++) {
        ...
        if (currentCol.toString() === previousCol.toString()) {
          let counter = 3;
          let valid = true;
      ...
      if (part2) {
        ...
        for (let j = 0; j < mirrorArray.length; j++) {
          ...
          for (let i = 0; i < mirrorArray[j].length; i++) {
            ...
            const result = findSplit(newMirrorArray);
            console.log(result);
            if (result > -1 && result !== firstFind) {
              console.log("breaking j " + j + " i " + i);
              newOutput += result;
              found = true;
              break;
            }
        ...
        if (newOutput === prevOutput) {
          console.log("nothing found for " + mirrorIndex);
        }
